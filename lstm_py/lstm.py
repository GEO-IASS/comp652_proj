...
def load_X(X_signals_paths):
    ...
    with open(X_signals_paths[0]) as f:
     for line in f:
        print [elt.strip() for elt in line.split(' ')]
        inner_list = [float(elt.strip()) for elt in line.split(' ')]
        # in alternative, if you need to use the file content as numbers
        # inner_list = [int(elt.strip()) for elt in line.split(',')]
        X_signals.append(inner_list)

    return np.array(X_signals)#np.transpose(np.array(X_signals), (1, 2, 0))
...
class Config(object):
    ...
    def __init__(self, X_train, X_test):
        ...
        self.n_steps = len(X_test)  # 128 time_steps per series
        ...
        # LSTM structure
        self.n_inputs = 8  # Features count is of 9: three 3D sensors features over time
        self.n_hidden = 5  # nb of neurons inside the neural network
