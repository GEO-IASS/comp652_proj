...
iter=1;
for no = 1:8 
    alltxt = dir(sprintf('curves/c%d/*.txt',no));
    for k=1:size(alltxt,1)
        %read measurements
        curvedata{iter}=importdata(sprintf('curves/c%d/%s',no,alltxt(k).name));
        %read location (lat&long) info
        f = fopen(sprintf('curves/c%d/%s',no,alltxt(k).name));
...
iter=1;
for no = 1:8 
    alltxt = dir(sprintf('lines/c%d/*.txt',no));
    for k=1:size(alltxt,1)
        %read measurements
        linedata{iter}=importdata(sprintf('lines/c%d/%s',no,alltxt(k).name));
        %read location (lat&long) info
        f = fopen(sprintf('lines/c%d/%s',no,alltxt(k).name));
        formatSpec = '%f %f %f %f %d %d';
...
%trainingcurve = [curvefeatures curvedistances];
%trainingline = [linefeatures linedistances];
...
trainingcombined(:,1:end-2) = (trainingcombined(:,1:end-1) - repmat(mean(trainingcombined(:,1:end-1)),size(trainingcombined,1),1))./repmat(std(trainingcombined(:,1:end-1)),size(trainingcombined,1),1);
